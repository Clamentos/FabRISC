...
#section(
    ...
    comment([
        ...
        The core ideas of this architecture are the use of variable length encodings of 4 and 6 byte instruction sizes along with shorter "compressed" ones to increase code density. The goal behind these is to use the long 6 byte format for instructions containing larger immediate values, while the 2 and 4 byte formats for instructions with smaller ones or registers only. The trade off is increased ISA capabilities and code density at the expense of misalignment and variable length encodings which can complicate the hardware implementation a bit. This ISA, all though not a "pure" RISC design with simple fixed length instructions and few addressing modes, resembles that philosophy for the most part skewing away from it in some areas, such as, being able to load, store, move multiple registers with a single instruction, more complex addressing modes, floating point transcendental operations and variable length encodings.
    ...
    subSection(
        ...
        tableWrapper([Technical term list.], table(
            ...
            [Abort, \ Transaction Abort \ Transaction Rollback \ Rollback], [Are used to refer to the act of abruptly stopping an ongoing memory transaction as well as invalidating, rolling back or undoing all of its changes. The first term is also used to broadly refer to the act of abruptly terminating an ongoing set of operations.],
