...
#section(
    ...
    subSection(
        ...
        comment([
            ...
            The hardware designers can choose from a variety of different sizes for the MMIO space, ranging from the minuscule 8B suitable for a tiny (toy-like) 8-bit implementation, all the way to 1TB suitable for more advanced machines that also implement a full blown operative system.
        ])
    ),
    ...
    subSection(
        ...
        [The DMA protocol or scheme implemented by the hardware designer must also take consistency into account since regular memory operations to different addresses are allowed to be done out-of-order. This means that fencing instructions must retain their effect from the point of view of the hart and IO devices. IO devices must ultimately provide similar fencing features as well and behave as if they were any other hart.],
        ...
        comment([
            ...
            IO coherence, as well as its consistency, are actually the main reasons of this subsection as a remainder that it needs to be considered during the development of the underlying microarchitecture, including the devices themselves via the use of atomic and fencing operations.
        ])
    )
)
