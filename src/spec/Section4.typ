...
#section(
    ...
    subSection(
        ...
        [All scalar addressing modes support two auto-update modes to help with sequential data structures: _post-increment_ and _pre-decrement_ (See section 7 for more information).],
    ...
    subSection(
        ...
        list(tight: false,
            ...
            [*Read-modify-write instructions:* _These instructions provide several atomic read-modify-write operations on single variables, such as atomic addition (see section 7 for more information) that can generally scale better than using CAS-based algorithms._]
        ),
        ...
        subSubSection(
            ...
            align(center, table(
                ...
                [TBEG], [*Transaction Begin*: \ Causes the hart that executed this instruction to start monitoring accesses by other harts via the coherence protocol as well as incrementing the nesting counter by one, thus starting a transaction. If the nesting counter was zero prior to executing this instruction, the hart must enter into transactional mode. This instruction will write the outcome status of its execution in the specified destination register.],
        ...
        comment([
            ...
            FabRISC also provides a small set of simple read-modify-write type operations, which can ease the implementation of lock-free datastructures and algorithms.
    ...
    subSection(
        ...
        comment([
            ...
            The memory consistency model i wanted to utilize was a very relaxed one to allow all kinds of performance optimization to take place inside the system. However one has to provide some sort of restrictions, effectively special memory operations, to avoid obscure edge cases that can cause erroneous processing if the hart can execute memory instructions out of order.
